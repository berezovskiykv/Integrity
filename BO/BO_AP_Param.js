 ...
 */
class AnalogBOPopup extends ObservablePopup {
    ...
    setupSignalsPath() {
        ...
        this.config.Sensorname = accessData.stringValue(`${this.rootPath}.val.ID`);
        let descr = accessData.stringValue(`${this.rootPath}.val.Description`).replace("Значение"," ")
        ...
        accessData.stringValue(`${this.rootPath}.val.FracDigits`);
    }
...
//#region PublishFunctions
    ...
    publish_updateValue(object) {
        if (!object.observerAction) {
            object.observerAction = publisher.register([`${this.valuePath}.val`, 
                                                        this.statePath
                                                    ], 
                (newValue) => {this.updateValue(newValue.value, `${this.valuePath}.val`, this.statePath, this, object)})
        }
        else {}
    }
    //#endregion
...
//#region PopupFunctions
    ...
    ModeClick(object, objectName, prefix) {
        if (getSignalQuality(this.statePath) && !this.processStateBits()[prefix]) {
            runAccessBox(object.select, objectName + '.select.click', {postfix:``, codes: `${this.config.Code}.${this.config.cmd_on}`, inputTag: this.comTag})
        }
        else {
            runAccessBox(object.select, objectName + '.select.click', {postfix:``, codes: `${this.config.Code}.${this.config.cmd_off}`, inputTag: this.comTag})
        }
    }

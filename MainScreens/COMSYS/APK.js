...
class APK_dp extends ObservableObjectLsu {
   
    setupSignalPaths() {
        this.valuePath = `${this.config.rootPath}`;
        this.statePath = `${this.config.rootPath}.${this.config.bit}`;
        this.config.Description = `${this.statePath}.Description`
    ...
    updateText() {
        this.object.setStringValue(accessData.stringValue(this.config.Description), "id.Text");
        return true;
    }
...
function apk_dp(object, bit) {
    if (!object.parameter) {
        object.name = objectName;
        object.parameter = new APK_dp(object, {bit : bit});
    }

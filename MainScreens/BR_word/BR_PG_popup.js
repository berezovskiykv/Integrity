      ...
      //#region object_TextState
        publish_updateTextState(object, bit) {
            if (!object.observerAction2) {
              ...
              //#region object_TextState
        publish_updateTextState2(object, bit) {
            let tag;
            if(accessData.stringValue(`${this.statePath}.ID`).includes("3.2"))
                tag = 'KSPG.BR.Share_SHUPG2';
            else
                tag = 'KSPG.BR.Share_SHUPG1';
            if (!object.observerAction2) {
                object.observerAction2 = publisher.register([`${tag}.${bit}`
                                                        ], 
                    (newValue) => {this.updateTextState2(newValue.value, newValue.tag, this, object, bit, tag)})
            }
            else {}
        }
    ...
    publish_updateAlarms(object, prefix) {
        if (!object.observerAction) {
            object.observerAction = publisher.register([`${this.valuePath}.${prefix}`
                ...
                //#region object_alarms
    publish_updateTEN(object, prefix, bit) {
        let tag;
        if(accessData.stringValue(`${this.statePath}.ID`).includes("3.2"))
            tag = 'KSPG.BR.Share.SHUPG2';
        else
            tag = 'KSPG.BR.Share.SHUPG1';
        if (!object.observerAction) {
            object.observerAction = publisher.register([`${tag}.${prefix}`
                                                    ],
                (newValue) => {this.updateTEN(newValue.value, newValue.tag, prefix, tag, bit, this, object)})
            }
        else {}
    }
    //#region PublishFunctions
...
//#region PopupFunctions
    ...
    updateValue(value, path, context, object, bit) {
        ...
        updateTextState2(value, state, context, object, bit, tag) {
    ...
    updateAlarms(value, flt, prefix, context, object) {
        if(getSignalQuality(this.statePath)) {
            ...
            RGBAColoring(object.text, colors.SERVICE.Badqual.text, "TextColor")
              ...
              //аварии
    updateTEN(value, flt, prefix, tag, bit, context, object) {
        if(getSignalQuality(this.statePath)) {
            if(accessData.boolValue(`${tag}.${prefix}.${bit}`))
                RGBAColoring(object.indicator, this.DetColor(object,`${tag}.${prefix}.${bit}`,context), "FillColor")
            else
                RGBAColoring(object.indicator, colors.SERVICE.Flt.inact, "FillColor")
            RGBAColoring(object, colors.SERVICE.Goodqual.text, "text.TextColor");
            object.setStringValue(accessData.stringValue(`${tag}.${prefix}.${bit}.Description`), "text.Text");
        }
        else {
            RGBAColoring(object.indicator, colors.SERVICE.Badqual.field, "FillColor")
            RGBAColoring(object.text, colors.SERVICE.Badqual.text, "TextColor")
